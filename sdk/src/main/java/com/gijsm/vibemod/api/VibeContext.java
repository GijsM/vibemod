 ...
 *
 * <p>This is the Paper flavor of the v2 contract (ARCHITECTURE-V2 §4.1). The
 * Fabric/NeoForge flavor shares this FQCN with Mojang-typed members and the
 * same semantics; generated code compiles against whichever flavor the host
 * provides.
 */
public interface VibeContext {
    ...
    // Config reads are thread-safe (callable from client callbacks on loaders).
    ...
    // ---- client (loader hosts with a physical client; no-op on Paper and dedicated servers) ----

    /** True when this process has a physical client (singleplayer / LAN host). Always false on Paper. */
     ...
     * revoked with the mod. No-op on dedicated servers and on Paper, so mods
     * declaring client features degrade gracefully. Client callbacks run on
     * the RENDER thread and must only use {@link ClientContext} state — never
     * server state (see the ClientContext javadoc).
     */
    default void client(Consumer<ClientContext> setup) {
    }
}
