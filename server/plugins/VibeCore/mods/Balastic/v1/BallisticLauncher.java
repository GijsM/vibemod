...
/** Fires a random 3D line of explosions from the sky down onto a target player. */
public final class BallisticLauncher {
    ...
    public void launch(Player target) {
        ...
        holder[0] = ctx.repeat(0L, STEP_PERIOD_TICKS, () -> {
            Player livePlayer = target.isOnline() ? target : null;
            World liveWorld = world.getUID() != null ? world : null;
            if (liveWorld == null) {
                if (holder[0] != null) {
                    holder[0].cancel();
                }
            ...
            if (progress[0] >= TOTAL_STEPS) {
                ...
                if (livePlayer != null) {
                    livePlayer.playSound(livePlayer.getLocation(), Sound.ENTITY_LIGHTNING_BOLT_IMPACT, 1.0f, 1.0f);
                }
                if (holder[0] != null) {
                    holder[0].cancel();
                }
