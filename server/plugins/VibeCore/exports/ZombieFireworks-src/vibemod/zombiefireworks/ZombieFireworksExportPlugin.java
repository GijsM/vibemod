...
/**
 * Standalone export wrapper for VibeMine mod "ZombieFireworks": a real Paper plugin
 ...
 */
public final class ZombieFireworksExportPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            this.mod = new vibemod.zombiefireworks.ZombieFireworks();
            this.mod.onEnable(ctx);
        } catch (Exception e) {
            getLogger().severe("Failed to enable mod ZombieFireworks: " + e);
    ...
    private static final class StandaloneContext implements VibeContext {
        private final ZombieFireworksExportPlugin plugin;

        StandaloneContext(ZombieFireworksExportPlugin plugin) {
            this.plugin = plugin;
        }
        ...
        public void command(String name, String description, ModCommandHandler handler) {
            try {
                ...
                Bukkit.getCommandMap().register("zombiefireworks", cmd);
        ...
        public boolean configBool(String key) {
            switch (key) {
                case "enabled": return plugin.getConfig().getBoolean(key, true);
                default: return plugin.getConfig().getBoolean(key, false);
            }
        }
        ...
        public long configInt(String key) {
            switch (key) {
                case "effects-per-firework": return plugin.getConfig().getLong(key, 1L);
                case "firework-power": return plugin.getConfig().getLong(key, 0L);
                case "launch-delay-ticks": return plugin.getConfig().getLong(key, 2L);
        ...
        public double configDouble(String key) {
            switch (key) {
                case "spawn-chance": return plugin.getConfig().getDouble(key, 1.0);
                case "flicker-chance": return plugin.getConfig().getDouble(key, 0.5);
                case "trail-chance": return plugin.getConfig().getDouble(key, 0.5);
                case "sound-volume": return plugin.getConfig().getDouble(key, 1.0);
                default: return plugin.getConfig().getDouble(key, 0.0);
            }
        }
        ...
        public String configString(String key) {
            switch (key) {
                case "color-mode": return plugin.getConfig().getString(key, "random");
                default: return plugin.getConfig().getString(key, "");
            }
        }
